...
// Gravity Force Field 
//
GravityParticleForce::GravityParticleForce(const ofVec3f &g) {
	gravity = g;
}

void GravityParticleForce::updateForce(Particle * particle) {
	//
	// f = mg
	//
...
//
TurbulenceParticleForce::TurbulenceParticleForce(const ofVec3f &min, const ofVec3f &max) {
	tmin = min;
	tmax = max;
}

void TurbulenceParticleForce::updateForce(Particle * particle) {
	//
